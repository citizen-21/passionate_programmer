...
<div class="container" >
		...
		font-size: 40px;
		font-weight: bold;
		"><?php echo "<span> " . $_SESSION['name'] . "</span>"; ?>'s  Order</p>
		</div>
   ...
   <table class="table content-table" style="margin-top: -3rem;">
      ...
      <?php
        ...
        $query="SELECT * FROM `order_manager` WHERE `cust_id`= '".$_SESSION['id']."' ";
        $user_result=mysqli_query($con,$query);
        while($user_fetch=mysqli_fetch_assoc($user_result))
        {
            if($user_fetch[order_status] == "Pending" || $user_fetch[order_status] == "In Progress" || $user_fetch[order_status] == "Out For Delivery" || $user_fetch[order_status] == "")
            {
            echo"
            <thead>
                <tr>
